...
if(isset($_POST['bookingId']) && !empty($_POST['bookingId'])) {
    ...
    $sql = "UPDATE bookings SET status = 'Approved' WHERE booking_id = $bookingId";
    if ($conn->query($sql) === TRUE) {
        ...
        if ($userIdResult->num_rows > 0) {
            ...
            if ($userExists->num_rows > 0) {
                ...
                $notificationMessage = 'Your booking has been approved. You can now proceed with your plans. Thank you!';
                ...
                if ($conn->query($insertNotificationQuery) === TRUE) {
                    ...
                    $response['message'] = 'Booking Approved successfully and notification sent!';
                    echo json_encode($response);
                } else {
                    // Error saving notification
                    $response['success'] = false;
                    $response['message'] = 'Booking Approved successfully, but failed to save notification: ' . $conn->error;
                    echo json_encode($response);
                }
            } else {
